// src/components/blog-post/SocialSharing.tsx
'use client';
...
const SocialSharing = ({ blog }: SocialSharingProps) => {
  ...
  // `relative group` enables the hover tooltip; the tooltip span is positioned against it
  const iconClass =
    'relative group p-2 rounded-full bg-gray-100 hover:bg-brand-blue hover:text-white transition-colors duration-300';

  // Tooltip: hidden by default, fades in on hover/focus of the parent group
  const tooltipClass =
    'pointer-events-none absolute -top-9 left-1/2 -translate-x-1/2 whitespace-nowrap rounded bg-gray-900 px-2 py-1 text-xs text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100 group-focus-visible:opacity-100';
